...
pub fn emit_cert(o: &mut dyn std::io::Write, sequoia: &Sequoia, cert: &openpgp::Cert)
                 -> Result<()>
{
    emit_cert_userid_str(
        o, cert, &sequoia.best_userid(cert, true).display().to_string())
}

/// Emits a certificate header.
pub fn emit_cert_indent(o: &mut dyn std::io::Write,
                        indent: &str,
                        sequoia: &Sequoia, cert: &openpgp::Cert)
                        -> Result<()>
{
    emit_cert_userid_str_indent(
        o, indent, cert, &sequoia.best_userid(cert, true).display().to_string())
}
