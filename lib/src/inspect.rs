...
use sequoia_openpgp::parse::buffered_reader::{BufferedReader, Dup};
...
use crate::packet::dump::PacketDumper;
...
pub fn inspect<'a, R>(sq: &Sequoia,
    ...
    -> Result<Kind>
where
    R: BufferedReader<openpgp::parse::Cookie> + 'a,
{
    let mut ppr =
...
fn inspect_bare_revocation(sq: &Sequoia,
...
fn inspect_signatures(sq: &Sequoia,
                      output: &mut dyn io::Write,
...
fn emit_issuers<F>(sq: &Sequoia, mut emit: F, sig: &Signature)
                   -> Result<()>
where
...
fn inspect_certifications<'a, A>(sq: &Sequoia,
                                 output: &mut dyn io::Write,
                                 certs: A,
                                 print_certifications: bool)
    -> Result<()>
...
impl Kind {
    ...
    /// consumed.
    pub fn identify<'a, T>(sq: &Sequoia, input: T)
